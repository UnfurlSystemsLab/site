...
| `/research/dcp/part-7-living-assembly` | Part 7: The Living Assembly | Observe, test, trace, audit, and evolve assemblies at runtime. | Explore Studio and Fiber Labs |
...
| `/labs/fiber` | Unfurl Fiber | Explain the connection and runtime-facing layer as currently defined. Clearly label exploratory areas. | Read the current direction or open the repository |
...
|---|---|---|
| `/labs/dcp` | `dcp` | Protocol purpose, specification status, quick example, validation and build instructions |
| `/labs/fabric` | `unfurl-fabric` | Compiler role, architecture, local run, and contract workflow |
| `/labs/studio` | `unfurl-studio` or `unfurl-ui` | Human harness, screenshots, local setup, and relationship to Fabric and DCP |
| `/labs/fiber` | `unfurl-fiber` | Current definition, scope, experimental status, and examples |
...
Flow and Foundry repositories should remain outside the promoted public repository set for now unless a later publishing decision explicitly adds them to the website and the `UnfurSystemsLab` public surface.
...
| Living Assembly Article | Studio or Fiber Lab | Inspect runtime observation and operational concepts |
