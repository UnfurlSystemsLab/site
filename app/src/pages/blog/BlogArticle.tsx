      ...
      <header style={{ borderBottom: '1px solid #e5e2da' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '36px 24px 44px' }}>
          <nav aria-label="Breadcrumb" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 32 }}>
            ...
            <span style={{ font: "400 12px/1 'IBM Plex Mono',monospace", color: '#c2beb2' }}>/</span>
            <span style={{ font: "400 12px/1 'IBM Plex Mono',monospace", color: '#3a382f' }}>Essay {String(post.number).padStart(2, '0')}</span>
          </nav>
          ...
          <h1 style={{ font: "300 clamp(28px,4.2vw,40px)/1.2 'Source Serif 4',serif", letterSpacing: '-0.02em', color: '#1c1b19', margin: '0 0 18px', textWrap: 'pretty' }}>
            {post.title}
          </h1>
          <p style={{ font: "400 18px/1.55 'Source Serif 4',serif", color: '#5c594f', margin: '0 0 24px', textWrap: 'pretty' }}>{post.description}</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 20px', alignItems: 'center', paddingTop: 20, borderTop: '1px solid #e5e2da' }}>
            <span style={{ font: "400 13px/1 'IBM Plex Sans',sans-serif", color: '#3a382f' }}>{publication.author}</span>
            <span style={{ font: "400 12px/1.5 'IBM Plex Mono',monospace", color: '#8a877f' }}>{publication.authorBio}</span>
          </div>
        </div>
      </header>
      ...
      {post.coverImage && (
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 24px' }}>
          <img
            ...
            style={{ width: '100%', height: 'auto', borderRadius: 3, border: '1px solid #e5e2da', display: 'block', margin: '32px 0 0' }}
          />
        </div>
      )}
          ...
          {post.tags.map((t) => (
            <li key={t}>
              <button
                ...
                style={{ border: '1px solid #ddd9ce', background: '#fff', borderRadius: 2, padding: '5px 10px', font: "500 11px/1 'IBM Plex Mono',monospace", color: '#5c594f', cursor: 'pointer' }}
              >
                {t}
              </button>
            </li>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 16 }}>
          {previous && (
            ...
            >
              ...
              <p style={{ font: "400 17px/1.35 'Source Serif 4',serif", color: '#1c1b19', margin: 0 }}>{previous.title}</p>
            </button>
          )}
          {next && (
            <button
            ...
            >
              ...
              <p style={{ font: "400 17px/1.35 'Source Serif 4',serif", color: '#1c1b19', margin: 0 }}>{next.title}</p>
            </button>
          )}
        </div>
      </nav>
...
function BlockView({ block }: { block: ContentBlock }) {
  switch (block.type) {
    ...
    case 'code':
      return (
        <div style={{ border: '1px solid #e5e2da', background: '#1c1b19', borderRadius: 3, padding: '18px 20px', margin: '0 0 24px', overflowX: 'auto' }}>
          <pre style={{ margin: 0, font: "400 13px/1.6 'IBM Plex Mono',monospace", color: '#e8e5dd' }}>
