...
export default function BlogArticleCard({ post, compact = false }: { post: Post; compact?: boolean }) {
  ...
  return (
    ...
    >
      <div>
        <span style={{ font: "500 10px/1 'IBM Plex Mono',monospace", letterSpacing: '.12em', textTransform: 'uppercase', color: '#073eab' }}>
          Essay {number} / 12 · {post.track}
        </span>
        <h3 style={{ font: `400 ${compact ? 19 : 22}px/1.3 'Source Serif 4',serif`, color: '#0f172a', margin: '8px 0 0' }}>{post.title}</h3>
      </div>
      <div>
        <p style={{ font: "400 15px/1.6 'Source Serif 4',serif", color: '#475569', margin: 0 }}>{post.excerpt}</p>
